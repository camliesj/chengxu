...
- 跨端登录视觉统一已完成：网页与 Windows 继续共用“智纬” `AccessGate`；Android 安装二维码从登录表单外部移入“客户端下载”弹窗的 Android 下载卡，并与同一 HTTPS release URL 的下载按钮并列。登录页新增无文字的浅冰蓝抽象车辆维修背景图 `public/brand/zhiwei-login-background.png`；移动宽度下下载卡保持单列。规格/计划分别为 `docs/superpowers/specs/2026-07-28-unified-login-download-background-design.md` 和 `docs/superpowers/plans/2026-07-28-unified-login-download-background.md`。
- 该视觉包不修改 D1/Room schema、业务 API、权限或发布元数据。已在用户授权后部署到 Pages 生产 `main` 分支，预览地址为 `https://7c18a882.chengxu.pages.dev`，正式域名为 `https://chengxu.pages.dev`；只读核验首页、背景图和 `0.1.1` APK 均为 HTTP 200，APK MIME 为 `application/vnd.android.package-archive`，`GET /api/client-releases` 返回 JSON 正常。未执行 D1 migration 或写入业务数据；Android 发布元数据仍为 `0.1.0`，没有擅自触发 `0.1.1` 客户端更新。回归验证：Node `npm.cmd test` 204/204、`npm.cmd run build` 成功；`test/zhiweiBrandContract.test.mjs` 锁定背景资源、外置二维码移除和弹窗内二维码条件。
