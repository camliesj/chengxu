...
- 已确认跨端登录视觉统一：网页与 Windows 共用的 `AccessGate` 保持“智纬”品牌；Android 二维码将从登录表单外移入“客户端下载”弹窗的 Android 下载卡；新增一张无文字的浅冰蓝抽象车辆维修背景图供登录页复用。规格为 `docs/superpowers/specs/2026-07-28-unified-login-download-background-design.md`，尚未改动实现，等待用户审阅规格后再写计划。
