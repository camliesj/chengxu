...
### Android UI 修复：全局状态栏安全区（已完成，待真机视觉验收）

- 真机截图确认 Android 15+ 边到边布局下，工单详情标题与返回区域覆盖系统状态栏。现已在 `AutoserviceApp` 根 Compose 容器统一消费顶部 `statusBarsPadding()`：登录、恢复、五栏和全部子路由均从系统状态栏下方开始；底部 NavigationBar 未增加 inset，因此不会制造双重底部留白。
- 测试先行：新增 `AutoserviceSafeAreaTest`，在实现前 `:app:compileDebugAndroidTestKotlin` 精确因缺少 `AutoserviceRootLayout` 失败；实现后测试源码编译成功。按用户“不启动模拟器”要求，Compose 测试仅编译，顶部实际几何由真机验收清单确认。
- 最终无设备门禁 `:app:testDebugUnitTest :app:compileDebugAndroidTestKotlin :app:lintDebug :app:assembleDebug --rerun-tasks` 为 `BUILD SUCCESSFUL`（68 tasks）；未启动模拟器或运行 connected 测试。新 APK 为 `dist/releases/android/autoservice-android-debug-0.1.0.apk`，19,802,837 bytes，SHA-256 `80A3B49FCD679B811D9CB9911447EE2EE24F8276257C04C050FFA033B8B05A07`，v2 签名已验证。
...
- 仓储根据刚读取的服务端状态前置拒绝非法写入：仅“待结算”可结算、仅“已结算”可返结算；新增 RED/GREEN JVM 回归测试确认不会发出 HTTP 请求。本次无数据库结构变化。
...
- Android 手动检查更新 Task 3 已完成：认证后的“我的”页新增与既有卡片一致的“版本与更新”区域，显示本机版本、手动检查、远端版本说明、下载进度、失败重试和下载完成后的安装入口。更新 ViewModel 与业务 ViewModel 隔离；UI 仅派发操作，FileProvider URI、未知来源授权和系统安装 Intent 由 Android 平台安装器处理。`UpdateViewModelTest`、相关下载器测试、Debug Kotlin 与 Android test source 离线编译均通过。下一步递增版本并完整构建、归档、签名校验 APK。
- Android 手动检查更新已完整交付：`0.1.1`（versionCode 2）在“我的”页手动读取 `/api/client-releases`；仅高于本机的三段数字 HTTPS Android 版本可弹出说明并下载。APK 下载至应用专属 `Download/`，先写 `.part` 后原子替换；完成后通过受限 FileProvider 调起系统安装器，未允许未知来源安装时打开系统授权页。无自动检查、无强制更新、无 D1/Room migration，业务 API/权限/数据均未改变。
- 跨端登录视觉统一已完成：网页与 Windows 继续共用“智纬” `AccessGate`；Android 安装二维码从登录表单外部移入“客户端下载”弹窗的 Android 下载卡，并与同一 HTTPS release URL 的下载按钮并列。登录页新增无文字的浅冰蓝抽象车辆维修背景图 `public/brand/zhiwei-login-background.png`，已降低覆盖遮罩以保留右侧车辆线稿；移动宽度下下载卡保持单列。已发布 Android 卡改为“扫码或下载 Android 安装包”，`???? Android ???` 类损坏说明由前端隐藏；Windows/Android 下载按钮统一为“下载客户端”及 46px 高度、8px 圆角。规格/计划分别为 `docs/superpowers/specs/2026-07-28-unified-login-download-background-design.md` 和 `docs/superpowers/plans/2026-07-28-unified-login-download-background.md`。
- 该视觉包不修改 D1/Room schema、业务 API、权限或发布元数据。已在用户授权后部署到 Pages 生产 `main` 分支，最新预览地址为 `https://758ef6ac.chengxu.pages.dev`，正式域名为 `https://chengxu.pages.dev`；只读核验首页、背景图和 `0.1.1` APK 均为 HTTP 200，APK MIME 为 `application/vnd.android.package-archive`，`GET /api/client-releases` 返回 JSON 正常。未执行 D1 migration 或写入业务数据；Android 发布元数据仍为 `0.1.0`，没有擅自触发 `0.1.1` 客户端更新。回归验证：Node `npm.cmd test` 205/205、`npm.cmd run build` 成功；`test/zhiweiBrandContract.test.mjs` 锁定背景资源、外置二维码移除和弹窗内二维码条件，`clientReleaseLogic` 与 Android QR 契约锁定状态说明、乱码兜底和统一下载按钮。
- 最终 Android 离线门禁：`:app:testDebugUnitTest`、`:app:compileDebugAndroidTestKotlin`、`:app:lintDebug` 和 `:app:assembleDebug` 均 `BUILD SUCCESSFUL`。新 APK 归档为 `public/downloads/zhiwei-car-service_0.1.1.apk` 与 `dist/releases/android/autoservice-android-debug-0.1.1.apk`，大小 20,641,703 bytes，SHA-256 `C6306EE495ED20F29170972B2B5F782842D3A61CEDBD97046A0034FEFA407890`；`apksigner` 确认 v2=true，`aapt` 确认包名 `com.chengxu.autoservice`、versionName `0.1.1`、versionCode 2、标签“智纬”。未启动模拟器。后续需要将该 APK 作为 `0.1.1` 发布元数据上传/设置后，再用仍为 `0.1.0` 的真机验收完整更新下载与系统安装流程。
- Windows 0.1.4 安装包已正式发布并完成端到端完整性核验：`智纬_0.1.4_x64-setup.exe`，45,270,758 bytes（43.17 MB），SHA-256 `49A9197C506088200B9F0A8BD35055D2CA1FCED6AFD79A61C69F8F077455A8EC`。受控对象键为 `releases/windows/0.1.4/chengxu_0.1.4_x64-setup.exe`，公开下载地址为 `https://chengxu.pages.dev/api/client-downloads/windows/0.1.4/%E6%99%BA%E7%BA%AC_0.1.4_x64-setup.exe`；生产下载包哈希与本地 NSIS 签名构建完全一致。
- Windows 桌面端版本已从 `0.1.3` 递增到 `0.1.4`，并保持 npm、Cargo manifest、Cargo lock 与 Tauri 配置一致。Windows release profile 启用 `strip`、thin LTO、单 codegen unit、`opt-level = "s"` 与 `panic = "abort"`；虽然该包仍为 43.17 MB，发布接口现按平台限额：Windows 64 MiB，Android 继续严格限制 25 MiB。回归测试锁定该边界，避免 APK 上限被意外放宽。
...
- 定向 JVM 测试、全量 JVM 测试和 Android 测试源码编译均为 `BUILD SUCCESSFUL`。尚未递增版本、构建签名 APK 或上传发布渠道；本检查点不可作为新移动端安装包发布。
