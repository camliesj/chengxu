...
- 该视觉包不修改 D1/Room schema、业务 API、权限或发布元数据，未部署 Pages/D1。回归验证：Node `npm.cmd test` 204/204、`npm.cmd run build` 成功；`test/zhiweiBrandContract.test.mjs` 锁定背景资源、外置二维码移除和弹窗内二维码条件。
